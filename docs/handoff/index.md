...
- **2026-09-09 — Tile-streaming repair approved and in progress.** User approved the discussed cache/retry plan after normal 0.2.0 release. Spec: docs/spec/FEATURE_SPEC_tile_streaming.md. Native gpt-6-astra/high worker `/root/tile_streaming`, lone serial writer in main; Hanuman absent. Parent owns docs, native tests and release. Worker brief `/tmp/aviadilo-tile-streaming/worker-brief.md`.
- Parent baseline on installed public 0.2.0 confirms known cached tiles get429busy when eight cold requests occupy a single regular user's gateway slots. No extra upstream call occurred for that probe. Native injected one429 per tile leaves15 blanks with exactly one attempt each after6seconds, without pan/reload. Actual household incident source still unconfirmed. Native fixture uses installed-only synthetic OSM hold controls; no external provider traffic. Evidence `/tmp/aviadilo-tile-streaming`.
- Implement bounded cache-hit admission/fair cold queue, transient Retry-After/backoff and fresh basemap-only 10minute idle cache (32MiB/128entries); no photos/idle network or upstream pacing increase. Expected delivery0.2.1-dev.1, stable0.2.0 staysLatest untilassessment. Test same-account browsers regardless of unanswered optional kiosk-login question.
- `docs/TODO.md` is dirty user intake; do not edit/stage/revert it. No .bishop changes observed. The original normal0.2.0 implementation remains accepted. Previous complete release evidence is in archive34 and `/tmp/aviadilo-release-020`. Do not move released tags. Native HA in `/tmp/aviadilo-slice8/ha` is currently running for parent baseline tests and must be stopped before replacement/sessionend; no household HA touched.
