...
10. Build the next kiosk development candidate as `0.2.0-dev.1` with the existing MIT/HACS packaging and channel-aware release workflow; do not move or replace `v0.1.0-dev.2` or the normal `v0.1.0` release. Verify the actual published ZIP and upgrade with retained entry/cache/v1 dashboard, then ask the user to assess the kiosk changes. Handle an already-used version through normal versioning, never by overwriting an existing release.
...
- 2026-09-08 — User prioritized repairing ordinary HACS updates. Release repair publishes current tested code as normal 0.1.0 with an attached ZIP; prerelease-only defaults had selected nonexistent commit ZIPs. This supersedes waiting for slice 6 to fix delivery. The next kiosk candidate moves to 0.2.0-dev.1 so it sorts after 0.1.0; the pending native-preview-remount decision remains unresolved.

- 2026-09-08 — User confirmed v0.1.0 HACS updating works, map tile caching/loading is great and editing no longer shows placeholders, then requested session wrap. Remaining status/weather information is scheduled for slice 5; theme/wind/card-v2 work is slice 4. This confirms the observed shipped behavior without explicitly answering the separate cross-remount client-continuity question.
