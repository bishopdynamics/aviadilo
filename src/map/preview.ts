...
export function previewEvents(
  ...
  now = Date.now(),
): import('../data/types').SnapshotEvent[] {
  const envelope = {
...
export function previewTileClient(): import('../layers/radar/controller').TileClient {
  return {
    async loadTile(tile, signal) {
      if (signal?.aborted) throw new DOMException('Canceled', 'AbortError');
      return syntheticTile(tile.frame).toDataURL('image/png');
    },
    releaseTile() {},
  };
}
